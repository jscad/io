...
test('serialize cag to json', function (t) {
  const cag10 = new CAG()
  const observed10 = serializer.serialize({}, cag10)

  t.deepEqual(observed10, expected10)

  const cag12 = CAG.rectangle()
  const observed12 = serializer.serialize({}, cag12)

  t.deepEqual(observed12, expected12)

  const cag13 = CAG.rectangle({radius: 5})
  const observed13 = serializer.serialize({}, cag12, cag13)
